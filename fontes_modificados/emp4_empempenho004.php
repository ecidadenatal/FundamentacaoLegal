...
if (isset($incluir)) {
  ...
  try {
    ...
    /* Plugin Cota Mensal Empenho - Parte 1 */
    ...
    if ($sqlerro == false && !empty($oEmpenhoFinanceiro)) {
      ...
      for ($i = 0; $i < $numrows09; $i++) {
        ...
        $sComplemento   = "Lançamento do Empenho " . $oEmpenhoFinanceiro->getNumero() . "/"
    ...
    if(!$sqlerro) {
      ...
      $totalEmpenhadoFundamentacao = $oDaoFundamentacaoLegalDesdobramentos->totalempenhadodesdobramento(null, $sWhere);
      ...
      if(!empty($totalEmpenhadoFundamentacao) && !empty($valorLimiteFundamentacao) && ($totalEmpenhadoFundamentacao > $valorLimiteFundamentacao)) {
        $sqlerro = true;
        $erro_msg = "O valor do empenho excede o limite para esta fundamentação.";
      }
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if ($sqlerro == false) {
      ...
      try {
        ...
        $iClassificacaoCredor = ClassificacaoCredor::getClassificacaoCredorPorEmpenho($oEmpenho);
        $lDispensaManual      = $classificacao_credor_combo == 1;
        if ($lDispensaManual) {
          $iClassificacaoCredor = ClassificacaoCredor::DISPENSA;
        }
    ...
    if ($sqlerro == false) {
      ...
      try {
        ...
        if ($lDispensaManual && empty($cc31_justificativa)) {
          throw new BusinessException('O campo Justificativa é de preenchimento obrigatório.');
        }
        $cc31_justificativa = ($iClassificacaoCredor == ClassificacaoCredor::DISPENSA) ? $cc31_justificativa : '';
        ClassificacaoCredor::vincularEmpenhoEmClassificacao($oEmpenho, $iClassificacaoCredor, $cc31_justificativa);
      } catch (Exception $oErro) {
    ...
    if ($opc == 2 && $sqlerro == false) {
      ...
      try {
        ...
        ClassificacaoCredor::validaParametros($oEmpenho,
                                              $e69_dtnota,
                                              $e69_dtrecebe,
                                              $e69_dtvencimento,
